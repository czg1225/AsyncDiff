...
def run_inference(rank, world_size, config):
    ...
    if config["strategy"] == "n3s2":
        from sdxl.n3s2 import AsyncDiffusionWorker
    elif config["strategy"] == "n2s1":
        from sdxl.n2s1 import AsyncDiffusionWorker
    elif config["strategy"] == "n3s1":
        from sdxl.n3s1 import AsyncDiffusionWorker
    elif config["strategy"] == "n4s1":
        from sdxl.n4s1 import AsyncDiffusionWorker
    elif config["strategy"]== "n2s2":
        from sdxl.n2s2 import AsyncDiffusionWorker
